...
export default function CustomizeDabbaSection() {
  ...
  const [portion, setPortion] = useState<'standard' | 'reduced'>('standard')
  ...
  const handleSubscribe = () => {
    ...
    const portionStr = encodeURIComponent(portion)

    router.push(
      `/checkout?plan=${selectedPlan}&likes=${likesStr}&dislikes=${dislikesStr}&slot=${deliverySlot}&categories=${categoriesStr}&portion=${portionStr}`
    )
  }

  return (
    <section id="customize" className="py-8 sm:py-12 bg-[#fdf0d5]/40 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center max-w-3xl mx-auto mb-8">
          <span className="inline-flex items-center rounded-full bg-orange-100 px-3 py-1 text-xs font-bold uppercase tracking-wider text-orange">
            Interactive Box Builder
          </span>
          <h2 className="text-2xl md:text-4xl font-extrabold text-gray-900 mt-2 leading-tight">
        ...
        <div className="grid lg:grid-cols-12 gap-6 items-start">
          ...
          <div className="lg:col-span-7 space-y-4">
            ...
            <div className="card p-5 md:p-6 bg-white border border-gray-150 shadow-soft">
              <h3 className="text-base font-extrabold text-gray-900 mb-4 flex items-center gap-2">
                <span className="w-6 h-6 rounded-lg bg-[#22c55e]/10 text-[#22c55e] flex items-center justify-center font-bold text-xs">1</span>
                Choose Daily Pack & Options
              </h3>

              {/* Plan selector */}
              <div className="mb-4">
                <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-2">Subscription Pack Type</label>
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                  {[
                    ...
                    >
                      <div className="font-extrabold text-[11px] leading-tight">{plan.label}</div>
                      <div className="text-[9px] opacity-75 mt-0.5">{plan.sub}</div>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="grid md:grid-cols-1 gap-4">
                <div>
                  <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-2">Preferred Delivery Slot</label>
                  <div className="flex gap-2">
                    {[
                      ...
                      <button
                        ...
                        type="button"
                        className={`flex-1 py-2 px-3 rounded-lg border text-center transition-all duration-200 cursor-pointer ${
              ...
              <div className="mt-4">
                <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-2">Dabba Quantity / Portion Size</label>
                <div className="flex gap-2">
                  {[
                    { id: 'standard', label: 'Standard Portion', desc: 'Full standard weight' },
                    { id: 'reduced', label: 'Reduced Portion', desc: 'Reduce 50-70g (saves ₹300)' }
                  ].map(item => (
                    <button
                      key={item.id}
                      onClick={() => setPortion(item.id as any)}
                      type="button"
                      className={`flex-1 py-2 px-3 rounded-lg border text-center transition-all duration-200 cursor-pointer ${
                        portion === item.id
                          ? 'border-[#22c55e] bg-[#22c55e]/5 text-gray-900 shadow-sm font-bold ring-2 ring-[#22c55e]/15'
              ...
              <div className="mt-4 border-t border-gray-100 pt-4 bg-green-50/60 rounded-xl p-3">
                <div className="flex items-start gap-2.5">
                  <span className="text-lg">💡</span>
                  <div>
                    <div className="font-bold text-xs text-gray-800">Want juice, oats, or salad too?</div>
            ...
            <div className="card p-5 md:p-6 bg-white border border-gray-150 shadow-soft">
              <h3 className="text-base font-extrabold text-gray-900 mb-3 flex items-center gap-2">
                <span className="w-6 h-6 rounded-lg bg-orange/10 text-orange flex items-center justify-center font-bold text-xs">2</span>
            ...
            <div className="card p-5 md:p-6 bg-white border border-gray-150 shadow-soft">
              <h3 className="text-base font-extrabold text-gray-900 mb-3 flex items-center gap-2">
                <span className="w-6 h-6 rounded-lg bg-blue-500/10 text-blue-500 flex items-center justify-center font-bold text-xs">3</span>
                ...
                {FRUIT_LIST.map(fruit => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      className={`p-2 rounded-xl border text-center transition-all duration-300 hover:shadow-sm cursor-pointer flex flex-col items-center justify-center min-h-[85px] ${bgClass}`}
                    >
                      <div className="text-2xl mb-1">{fruit.emoji}</div>
                      <div className="font-bold text-[10px] tracking-tight leading-tight line-clamp-1">{fruit.name}</div>
                      <div className="text-[8px] mt-1 font-bold uppercase tracking-wider opacity-60 flex items-center gap-0.5">
                        <span>{icon}</span>
                        {state}
                      </div>
                    </button>
          ...
          <div className="lg:col-span-5 sticky top-28 space-y-4">
            <div className="card p-5 md:p-6 bg-forest-dark text-white shadow-soft relative overflow-hidden">
              ...
              <div className="flex justify-between items-start mb-4">
                <div>
                  <span className="text-[9px] bg-orange text-white font-extrabold uppercase tracking-widest px-2.5 py-0.5 rounded-full">
                    Your Curated Box
                  </span>
                  <h3 className="text-lg font-bold mt-1.5">Dabba Simulation</h3>
                </div>
                <div className="text-2xl">📦</div>
              </div>

              {/* Simulation summary */}
              <div className="bg-white/10 rounded-xl p-4 border border-white/10 space-y-3 mb-5 text-xs">
                ...
                <div className="flex justify-between border-b border-white/15 pb-1.5">
                  ...
                  <span className="font-bold">
                    {selectedPlan === 'basic-pack' ? (portion === 'reduced' ? '250-300 gms (Reduced)' : '300-350 gms') :
                     selectedPlan === 'mini-pack' ? (portion === 'reduced' ? '350-400 gms (Reduced)' : '400-450 gms') :
                     selectedPlan === 'medium-pack' ? (portion === 'reduced' ? '450-500 gms (Reduced)' : '500-550 gms') :
                     selectedPlan === 'premium-pack' ? (portion === 'reduced' ? '650-700 gms (Reduced)' : '700-750 gms') :
                     selectedPlan === 'gym-protein-pack' ? (portion === 'reduced' ? 'Protein Salad (Reduced)' : 'High Protein Salad') :
                     selectedPlan === 'corn-chaat-pack' ? (portion === 'reduced' ? 'Corn Salad (Reduced)' : 'Corn Salad') :
                     (portion === 'reduced' ? 'Custom Portion (Reduced)' : 'Custom Portion (300-750g)')}
                  </span>
                </div>
                <div className="flex justify-between border-b border-white/15 pb-1.5">
                ...
                {/* Visual fruits list */}
                <div>
                  <span className="text-white/60 block mb-1.5 font-semibold">Included Fruits (curated):</span>
                  <div className="flex flex-wrap gap-1.5">
                    {FRUIT_LIST.filter(f => selectedCategories.includes(f.category) && !dislikes.includes(f.name))
                       .slice(0, 6)
                       .map(f => (
                        <span key={f.name} className="inline-flex items-center gap-1 bg-white/15 px-2 py-0.5 rounded-full text-[10px] font-medium">
                          {f.emoji} {f.name}
                        </span>
                      ))}
                    <span className="inline-flex items-center gap-1 bg-white/10 px-2 py-0.5 rounded-full text-[10px] font-semibold italic opacity-85">
                      + Seasonal Surprises
                    </span>
                  </div>
                </div>
                ...
                {/* Excluded dislikes */}
                {dislikes.length > 0 && (
                  <div className="text-[10px] bg-black/20 p-2 rounded-lg border border-white/5">
                    <span className="font-bold text-orange flex items-center gap-1">❌ Excluded Dislikes:</span>
                    <p className="text-white/70 mt-0.5">{dislikes.join(', ')}</p>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-2 mb-5 text-xs">
                ...
                {portion === 'reduced' && (
                  <div className="flex justify-between text-green-400 font-semibold">
                    <span>Portion Size Discount (50-70g less):</span>
                    <span>-₹300</span>
                  </div>
                )}
                <div className="flex justify-between text-white/70">
                ...
                <div className="border-t border-white/15 pt-3 flex justify-between items-baseline">
                  ...
                  <div className="text-right">
                    <span className="text-2xl font-extrabold text-[#22c55e]">₹{getPrice()}</span>
                    <span className="text-[10px] text-white/50 block font-normal mt-0.5">
                      / 26 days
                    </span>
                  </div>
                </div>
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-1.5 bg-[#22c55e] hover:bg-[#16a34a] text-white font-extrabold py-3 px-5 rounded-xl text-sm transition-all duration-200 hover:shadow-xl cursor-pointer"
              >
                <ShoppingBag size={16} />
                Subscribe Now
              ...
              <div className="mt-3 flex items-center gap-1.5 justify-center text-[11px] text-white/50">
            ...
            {/* Quick explanation */}
            <div className="bg-white rounded-xl p-4 border border-gray-150 flex items-start gap-2.5 shadow-sm">
              <Info className="text-forest size-4 shrink-0 mt-0.5" />
              <div>
                <h4 className="font-bold text-gray-900 text-xs">How Custom Boxes Work</h4>
