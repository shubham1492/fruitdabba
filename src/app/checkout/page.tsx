      ...
      try {
        const verify = await fetch('/api/razorpay/verify', {
          ...
          body: JSON.stringify({
            ...
            preferences: { likes, dislikes, deliverySlot: deliverySlotParam, categories }
          }),
        })
            ...
            <div className="flex bg-cream rounded-2xl p-1 mb-6">
              {(['login', 'register'] as const).map((m) => (
                <button key={m} type="button" onClick={() => setAuthMode(m)}
                  className={`flex-1 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200 ${authMode === m ? 'bg-forest text-white shadow-md' : 'text-gray-500 hover:text-gray-700'}`}>
                ...
                <div className="p-5 space-y-4">
                  ...
                  <div className="border-t border-gray-100 pt-3 space-y-2 text-sm">
                    <div className="flex justify-between text-gray-600">
                      <span>Subscription</span><span>₹{subtotal.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between text-gray-600">
                      <span>Delivery</span>
